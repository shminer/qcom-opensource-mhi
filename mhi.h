...
typedef struct osal_spinlock osal_spinlock;
...
{
	u32 dev_id; /*<-- Device ID as reported by the PCI device's config space -->*/
	u32 manufact_id; /*<-- Manufacturer as reported by the PCI devices' config space-->*/
	u32 mhi_ver; /*<-- Version of the MHI protocol supported by the dev -->*/
	u64 bar0_base; /*<-- BAR 0 Base Address -->*/
...
typedef enum MHI_DEVICE_STATE
{
	DEVICE_UINIT = 0x0,
	DEVICE_MHI = 0x1,
	DEVICE_BHI = 0x2,
	DEVICE_reserved = 0x80000000
}MHI_DEVICE_STATE;
...
{
	pcie_core_info core; /*<-- Core information about the pcie device -->*/
	atomic_t ref_count; /*<-- Number of clients currently registered with this device -->*/
	mhi_device_ctxt* mhi_ctxt; /*<-- Pointer to the mhi context for this device -->*/
	pci_dev* pcie_device; /*<-- Pointer to a device structure --*/
	bhi_ctxt_t bhi_ctxt;
}mhi_pcie_dev_info;

typedef struct mhi_pcie_devices
{
	/*<-- List of all pcie devices registered with the MHI driver -->*/
	mhi_pcie_dev_info device_list[MHI_MAX_SUPPORTED_DEVICES];
	s32 nr_of_devices; /*<-- Total Number of registered devices -->*/
}mhi_pcie_devices;

typedef struct mhi_handle
{
	u32 mhi_client; /*<-- MHI Client ID -->*/
	u32 registered_device; /*<-- Device which MHI client registered with -->*/
} mhi_handle;
...
/*MHI Command Ring Elements 7.4.2 */
...
{
	MHI_CHAN_STATE volatile mhi_chan_state;
	void* base;
	void* volatile wp;
	void* volatile rp;
	uintptr_t len;
...
typedef struct state_transition_history
{
	u32 M0_to_M1_cntr;
	u32 M1_to_M0_cntr;
	u32 M1_to_M2_cntr;
...
{
	//mhi_xfer_pkt* xfer_trb_list[MHI_MAX_CHANNELS][MAX_NR_TRBS_PER_CHAN + ELEMENT_GAP];
	mhi_xfer_pkt* xfer_trb_list[MHI_MAX_CHANNELS];
	mhi_event_pkt* ev_trb_list[EVENT_RINGS_ALLOCATED];
	mhi_cmd_pkt cmd_trb_list[NR_OF_CMD_RINGS][CMD_EL_PER_RING + ELEMENT_GAP];
...
typedef struct mhi_result
{
	...
	MHI_STATUS transaction_status; /*<-- Returned status of the last transaction -->*/
}mhi_result;
...
typedef struct mhi_client_cbs_t
{
...
struct mhi_device_ctxt
{
	...
	osal_mutex* mhi_chan_mutex;
	osal_spinlock* mhi_ev_spinlock_list;
	osal_mutex* mhi_cmd_mutex_list;
...
MHI_STATUS mhi_reset_all_thread_queues(mhi_device_ctxt* mhi_device);
...
MHI_STATUS mhi_init_device_ctxt(mhi_pcie_dev_info *dev_info, mhi_device_ctxt** mhi_device);
...
MHI_STATUS mhi_clean_init_stage(mhi_device_ctxt* mhi_device, MHI_INIT_ERROR_STAGE cleanup_stage);
MHI_STATUS mhi_init_sync(mhi_device_ctxt* mhi_device);
MHI_STATUS mhi_init_ctrl_zone(mhi_pcie_dev_info *dev_info,mhi_device_ctxt* device);

MHI_STATUS mhi_init_threads(mhi_device_ctxt* mhi_device);
MHI_STATUS mhi_init_events(mhi_device_ctxt *device);

MHI_STATUS mhi_reset(mhi_device_ctxt* device);
MHI_STATUS mhi_send_cmd(mhi_device_ctxt* dest_device, MHI_COMMAND which_cmd, u32 chan);

MHI_STATUS mhi_start(mhi_pcie_dev_info* new_devices, u32 nr_devices);
MHI_STATUS mhi_queue_tx_pkt(mhi_device_ctxt* device, MHI_CLIENT_CHANNEL chan,
		void* payload, size_t payload_size);
MHI_STATUS change_chan_state(mhi_device_ctxt* device, u32 chan_id, MHI_CHAN_STATE new_state);
MHI_STATUS mhi_reset_channel(mhi_device_ctxt* device, u32 chan_id);

MHI_STATUS mhi_cmd_ring_init(mhi_cmd_ctxt* cmd_ring_id, uintptr_t trb_list_phy, uintptr_t trb_list_virt,
		size_t el_per_ring, mhi_ring* ring);
MHI_STATUS parse_cmd_completion_event(mhi_device_ctxt* device, mhi_event_pkt* ev_pkt);
...
MHI_STATUS recycle_trb_and_ring(mhi_device_ctxt* device, mhi_ring* ring,
		MHI_RING_TYPE ring_type, u32 ring_index);
MHI_STATUS mhi_change_chan_state(mhi_device_ctxt* device, u32 chan_id, MHI_CHAN_STATE new_state);
...
/** Internal Driver/Device interfaces */
MHI_STATUS mhi_init_device(mhi_pcie_dev_info* new_device);
s32 mhi_get_device (u32 device_id);
MHI_STATUS mhi_process_event_ring(mhi_device_ctxt* device,u32 event_ring_index, u32 quota);

int mhi_send_packet(mhi_client_handle* client_handle, void* buf,
		u32 size, u32 is_userspace_buffer);
MHI_STATUS mhi_init_outbound(mhi_device_ctxt*,MHI_CLIENT_CHANNEL);
MHI_STATUS mhi_test_for_device_ready(mhi_device_ctxt* mhi_device);


/* Cots */
MHI_STATUS validate_ring_el_addr(mhi_ring* ring, uintptr_t addr);
void print_ring(mhi_ring* ring);
uintptr_t calc_free_ring_elements(mhi_ring* shadow_chan_ctxt);

MHI_STATUS validate_ev_el_addr (mhi_ring* ring, uintptr_t addr);

MHI_STATUS mhi_spawn_threads(mhi_device_ctxt* mhi_device);
...
MHI_STATUS mhi_open_channel(mhi_client_handle** client_handle,
		...
		mhi_client_cbs_t* cbs, void* user_data);
...
void mhi_poll_inbound(mhi_client_handle* client_handle, uintptr_t* buf ,size_t* buf_size);
...
typedef enum MHI_REQUEST_TYPE
{
	MHI_REQUEST_TX = 0x1,
	MHI_REQUEST_RX = 0x2,
	MHI_REQUEST_reserved = 0x80000000
}MHI_REQUEST_TYPE;
